...
namespace Xradiator.ViewModels
{
	...
	public class ViewDataViewModel : NotifyingClass
	{
		...
		private bool _ShowAllOK;
		private bool _ShowOutOfDateProjects;
								 ...
								 IBuildBuster imageBuster = null, IBuildBuster nameBuster = null)
		{
			...
			this.Projects = projects.Select(p => new ProjectStatusViewModel(p, vs, imageBuster, nameBuster)).ToList();
		}
		...
		public List<ProjectStatusViewModel> Projects
		{
			get
			{
				if (ShowOnlyBroken)
				{
					return _projects.Where(p => p.IsBroken).ToList();
				}
				...
				if (ShowOutOfDate)
				{
					...
					return _projects.Where(p =>
						highestBuildDate.Subtract(p.LastBuildTime).TotalMinutes > OutOfDateDifferenceInMinutes).ToList();
				}
		...
		/// <summary> Show the projects viewbox </summary>
		...
		/// <summary> Show the AllOK viewbox aka smiley </summary>
		public bool ShowAllOK
		{
			get => _ShowAllOK;
			set
			{
		...
		/// <summary> Show out of date projects only </summary>
		public bool ShowOutOfDateProjects
		{
			get => _ShowOutOfDateProjects;
			set
			{
		...
		{
			get => _outOfDateDifferenceInMinutes;
			set
			{
		...
		private void CalculateProperties()
		{
			...
			{
				OKPercentage = (decimal)AmountOK / AmountTotal;
				OKPercentage *= 100;
			...
			{
				var highestBuildDate = _projects.Max(d => d.LastBuildTime);
				var outOfSynchs = _projects
					.Where(p => highestBuildDate.Subtract(p.LastBuildTime).TotalMinutes > OutOfDateDifferenceInMinutes).ToList();

				if (outOfSynchs.Count == 0)
				{
					ShowAllOK = true;
					ShowOutOfDateProjects = false;
					ShowProjects = false;
				}
				else
				{
					ShowAllOK = false;
					ShowOutOfDateProjects = true;
					ShowProjects = false;
				}
			}
			else
			...
			AmountHeader = $"Project Count : {this.AmountTotal} ";
		}
	}
}
